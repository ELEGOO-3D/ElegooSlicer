...
bool PartPlate::check_outside(int obj_id, int instance_id, BoundingBoxf3* bounding_box)
{
	...
	if (instance_box.min.z() < SINKING_Z_THRESHOLD) {
		...
		if (plate_box.intersects(instance_box)) {
            ...
            double print_height = m_plater ? m_plater->build_volume().printable_height() : (double)m_height;
            const BuildVolume build_volume(get_shape(), print_height, m_extruder_areas, m_extruder_heights);
	...
	if (m_shape.size() == 4)
	{
		...
		if (m_plater) {
            if (auto preset_bundle = wxGetApp().preset_bundle; preset_bundle) {
                ...
                // 尝试获取自定义的 bed_texture_area
                auto edited_preset = preset_bundle->printers.get_edited_preset();
                DynamicPrintConfig        global_config        = edited_preset.config;
