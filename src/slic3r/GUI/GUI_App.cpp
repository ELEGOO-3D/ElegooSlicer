...
void GUI_App::init_webview_runtime()
{
    ...
    if (installed && WebView::CheckWebViewRuntime()) {
        ...
        wxMessageBox(_L("The Microsoft WebView2 Runtime could not be installed.\n"
                        ...
                        "Please install it manually from https://developer.microsoft.com/microsoft-edge/webview2/ and restart ElegooSlicer."),
...
//	SetAppDisplayName(SLIC3R_APP_NAME);
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("gui mode, Current OrcaSlicer Version %1% build %2%") % ELEGOOSLICER_VERSION % GIT_COMMIT_HASH;
...
//     Slic3r::debugf "wxWidgets version %s, Wx version %s\n", wxVERSION_STRING, wxVERSION;
    ...
    if(app_config->get("version") != ELEGOOSLICER_VERSION) {
        app_config->set("version", ELEGOOSLICER_VERSION);
    }
...
void GUI_App::UpdateDarkUI(wxWindow* window, bool highlited/* = false*/, bool just_font/* = false*/)
{
    ...
    if (m_is_dark_mode) {
        ...
        // * the background color of a button: #1677ff  -- 73
        if (bg_col != orig_col) {
...
bool GUI_App::load_language(wxString language, bool initial)
{
    ...
    if (!wxLocale::IsAvailable(locale_language_info->Language)) {
	    ...
	    wxString message = "Switching ElegooSlicer to language " + requested_language_code + " failed.";
