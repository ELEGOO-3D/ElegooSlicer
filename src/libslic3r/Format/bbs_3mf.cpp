...
#define _(s) Slic3r::I18N::translate(s)
    ...
    public:
        ...
        bool load_model_from_file(const std::string& filename, Model& model, PlateDataPtrs& plate_data_list, std::vector<Preset*>& project_presets, DynamicPrintConfig& config,
            ConfigSubstitutionContext& config_substitutions, LoadStrategy strategy, bool* is_bbl_3mf, bool* is_orca_3mf, std::string& generator, Semver& file_version, Import3mfProgressFn proFn = nullptr, BBLProject *project = nullptr, int plate_id = 0);
    ...
    bool _BBS_3MF_Importer::load_model_from_file(const std::string& filename, Model& model, PlateDataPtrs& plate_data_list, std::vector<Preset*>& project_presets, DynamicPrintConfig& config,
        ConfigSubstitutionContext& config_substitutions, LoadStrategy strategy, bool* is_bbl_3mf, bool* is_orca_3mf, std::string& generator, Semver& file_version, Import3mfProgressFn proFn, BBLProject *project, int plate_id)
    {
        m_version = 0;
...
//        model.adjust_min_z();
    ...
    */
            ...
            for (auto item : metadata_item_map) {
                ...
                if (item.first == BBL_APPLICATION_TAG) {
                    stream << " <" << METADATA_TAG << " name=\"" << ORCASLICER_TAG << "\">"
                           << xml_escape(ORCA_COMPATIBLE_VERSION) << "</" << METADATA_TAG << ">\n";
                }
            }
    ...
    bool _BBS_3MF_Exporter::_add_slice_info_config_file_to_archive(mz_zip_archive& archive, const Model& model, PlateDataPtrs& plate_data_list, const ObjectToObjectDataMap &objects_data, const DynamicPrintConfig& config)
    {
        ...
        stream << "    <" << SLICE_HEADER_ITEM_TAG << " " << KEY_ATTR << "=\"" << "OrcaSlicer-Version" << "\" " << VALUE_ATTR << "=\"" << ORCA_COMPATIBLE_VERSION << "\"/>\n";
        stream << "  </" << SLICE_HEADER_TAG << ">\n";
...
bool load_bbs_3mf(const char* path, DynamicPrintConfig* config, ConfigSubstitutionContext* config_substitutions, Model* model, PlateDataPtrs* plate_data_list, std::vector<Preset*>* project_presets,
                    bool* is_bbl_3mf, bool* is_orca_3mf, std::string& generator, Semver* file_version, Import3mfProgressFn proFn, LoadStrategy strategy, BBLProject *project, int plate_id)
{
    ...
    CNumericLocalesSetter locales_setter;
    _BBS_3MF_Importer importer;
    bool res = importer.load_model_from_file(path, *model, *plate_data_list, *project_presets, *config, *config_substitutions, strategy, is_bbl_3mf, is_orca_3mf, generator, *file_version, proFn, project, plate_id);
    importer.log_errors();
