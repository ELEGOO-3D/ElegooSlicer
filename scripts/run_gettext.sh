...
if $FULL_MODE; then
    ...
    if $has_sources; then
        ...
        generated_pot_file="${generated_i18n_dir}/ElegooSlicer.pot"
...
do
    ...
    if [ -f "$dir/ElegooSlicer_${lang}.po" ]; then
        if $FULL_MODE && [ -f "$pot_file" ]; then
            merged_po=$(mktemp)
            if ! msgmerge -N -o "$merged_po" "$dir/ElegooSlicer_${lang}.po" "$pot_file"; then
                echo "Error encountered with msgmerge command for language ${lang}."
                rm -f "$merged_po"
                exit 1
            fi

            if files_equal_ignoring_pot_date "$dir/ElegooSlicer_${lang}.po" "$merged_po"; then
                rm -f "$merged_po"
            else
                mv "$merged_po" "$dir/ElegooSlicer_${lang}.po"
            fi
        fi
