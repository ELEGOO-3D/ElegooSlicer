...
function usage() {
    echo "Usage: ./${SCRIPT_NAME} [-1][-b][-c][-C][-D][-E][-e][-F][-g][-h][-i][-j N][-l][-L][-p][-r][-s][-S][-t][-u][-w]"
    ...
    echo "   -S: upload ELF debug symbols to Sentry after build"
    ...
    echo "For a GitHub Actions-like Linux build locally, use './${SCRIPT_NAME} -g -istrL'"
    echo "Use './${SCRIPT_NAME} -gF -istrL' to rebuild the cached runner image first."
...
FORWARDED_ARGS=()
while getopts ":1j:bcCdDeEFghiprsStulLw" opt ; do
  case ${opt} in
    1 )
...
if [[ -n "${BUILD_ELEGOO}" ]] || [[ -n "${BUILD_TESTS}" ]] ; then
    ...
    if [[ $BUILD_CONFIG != Release ]] ; then
        BUILD_ARGS+=(-DCMAKE_BUILD_TYPE="${BUILD_CONFIG}" -DBBL_INTERNAL_TESTING=1 -DELEGOO_INTERNAL_TESTING=0)
    else
...
"${BUILD_ARGS[@]}"
	...
	print_and_run cmake --build $BUILD_DIR --config "${BUILD_CONFIG}" --target ElegooSlicer
	echo "Building ElegooSlicer_profile_validator .."
	print_and_run cmake --build $BUILD_DIR --config "${BUILD_CONFIG}" --target ElegooSlicer_profile_validator
...
function upload_pdb() {
    BUILD_OUT_DIR="${BUILD_DIR}/src/${BUILD_CONFIG}"
    INSTALL_INI="${BUILD_DIR}/install.ini"
    SENTRY_VER="unknown"
